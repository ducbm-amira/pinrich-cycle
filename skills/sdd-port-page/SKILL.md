---
name: sdd-port-page
description: "Port một trang legacy (estimate, Vue 3 + Express) sang FE mới SDD (estimate-client-sdd, Next.js 16 + React 19). 6 phase: scope → hiểu legacy → preflight env → build → wire data + edge-states → qa-verify. Dùng khi user nói 'port trang', 'port page', 'build lại trang ... ở FE mới', 'chuyển trang ... sang SDD', 'migrate trang ... sang Next'. KHÔNG trigger cho fix bug trang đã có (dùng bug-fix), QA thuần (dùng qa-verify), hay tạo trang mới không có legacy."
disable-model-invocation: false
---

Trả lời bằng tiếng Việt. **Tone mặc định: CONCISE** — trả lời thẳng, hỏi xác nhận khi mơ hồ chứ không giải thích dài. Enforce 6 phase theo thứ tự, không skip.
...
5. **Design ground truth** — đọc `v2/docs/design/tokens.json` + `components/` + rules `pinrich/` để biết token/component chuẩn sẽ dùng.
6. **Figma** — nếu có URL design, extract bằng `/extract-ds <url>` (hoặc đọc registry `v2/docs/design/figma-sources.json` nếu đã extract). Map layout/spacing/màu theo design, không theo Vue cũ.

**Output P1:** Bản đồ ngắn: view → state → endpoint(s) → các phép dẫn xuất cần port (liệt kê từng cái). *Nếu Design bundle:* kèm danh sách token/component chuẩn + nguồn design (Figma node / file đã extract).
...
- [ ] API môi trường `develop` còn sống (`https://<develop-api-host>/...`). Endpoint cần header browser (Origin/Referer/UA) — curl trần có thể trả 500.
...
> Nếu được `/pinrich-suite:pinrich-cycle` gọi: P5 chỉ smoke-check nhanh, để cycle lo `/pinrich-suite:qa-verify` + vòng lặp fail. P6 (verify-against-design) vẫn giữ vì cycle QA không bắt lệch design.
...
3. **Báo NOT-verified trung thực** — cái nào script không chứng minh được thì nói rõ là chưa verify, đừng tô hồng.
4. **Chỉ khi Design bundle — design review:** chạy `/review <figma-url>` (hoặc `/review-batch`) để check trang dựng xong so với PINRICH Design System + Practical UI. Fix vi phạm HIGH trước khi coi là xong.

**Output P5:** Báo cáo: verified / not-verified theo từng TC, kèm bằng chứng (screenshot/assertion). *Nếu Design bundle:* kèm kết quả design review (vi phạm + đã fix).
...
- `pinrich_design_check`: bắt **font out-of-scale, màu/surface, và `DESIGN-NUMFMT-001` (giá ≥1000万 thiếu phẩy)**.
...
**Gate done:** mọi finding **severity=error** của 3 lớp = 0 **VÀ** đã nhìn 2 ảnh thấy khớp. Còn error → fix rồi chạy lại. (Token-diff ở P3 chặn gốc; P6 chặn cái render đã lọt.)
