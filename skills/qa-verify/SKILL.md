...
```sh
docker run --rm mysql:8 mysql -h <RDS_ENDPOINT> -u admin -p<DB_PASSWORD> egent_data -t -e "<SQL>"
```
...
```sql
SELECT id, client_name, email, url_key, deal_type FROM url_share_user WHERE email = 'duc_bm@amira.vn';
```
